var sinabinding = { //QQ账号绑定
	init: function(){
		...
		$rqpassword.focus(function (){
			...
			$mqpassword.html('<div class="err_message"><span class="icon">请输入您的登陆密码。</span></div>');
		}).blur(function(){
			$(this).triggerHandler('keyup');
		}).keyup(function(){
			if($(this).val() == ''){
				$mqpassword.html('<div class="err_message error"><span class="icon">请输入您的登陆密码！</span></div>');
				return false;
		...
		$submit.click(function(){
			...
			if(!numError){
				$("#register").html("<span class='input_register'>注册中...</span>");
				$.ajax({
					type: "POST",
					global: false,// 禁用全局Ajax事件.
