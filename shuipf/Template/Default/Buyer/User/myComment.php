  ...
  <template file="Buyer/common/_head.php"/>
        <div class="myComment_wrap right">
            <div class="myComment_title clearfix">
                <a href="{:U('myComment')}" class="on left">我的评价</a>
                <a href="{:U('sellerComment')}" class="left">来自卖家的评价</a>
            </div>
            <table>
                <tr>
                    <th>
                ...
                <tr>
                    ...
                    <input type="hidden" value="{$vo.id}" name="commentid">
                    <td><h4>{$vo.content}</h4><span><?php echo date("Y-m-d H:s:i",$vo['addtime']) ?></span></td>
                    <td><h4>卖家：{$vo.shop_name}</h4><img src="{$config_siteurl}statics/zt/images/buyercenter/comment4.png" alt="" /></td>
                    <td><a href="">{$vo.product_name}</a href="{:U('Shop/Product/show',array('id'=>$vo['product_id']))}"><p><strong>{$vo.product_price}</strong>元</p></td>
                    <td><a href="{:U('pay_commentedit',array('order_sn'=>$vo['order_sn'],'id'=>$vo['id']))}">修改</a><a href="{:U('pay_commentdele',array('order_sn'=>$vo['order_sn'],'id'=>$vo['id']))}"  >删除</a></td>
