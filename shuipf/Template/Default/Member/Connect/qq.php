...
<title>QQ帐号连接 - {$Config.sitename}</title>
  ...
  <div class="bind_box">
    <div class="regMain" id="regMain">
      <div class="register_box_left">
        <div class="option"><span id="existing"><a href="javascript:;" onclick="$('#qqBinding').show();$('#regData').hide();$('#existing').addClass('on');$('#not').removeClass('on');">已有账号</a></span><span id="not"  class="on"><a href="javascript:;" onclick="$('#regData').show();$('#qqBinding').hide();$('#not').addClass('on');$('#existing').removeClass('on');">完善资料</a></span></div>
        <div class="title">
          <div class="name">HI, <b>{$connect.userinfo.nickname}</b>, 现在您可以用QQ帐号连接 {$Config.sitename} 了!</div>
        </div>
        <ul id="qqBinding" style="display:none;">
          <form onSubmit="return false;">
            <li><span  style="font-size:14px;color:#333;">将已有的账号与您的 QQ帐号 进行绑定！</span></li>
            <li>
              <div class="rM_title">登陆账号：</div>
              <div class="rM_input" >
...
<script type="text/javascript" src="{$model_extresdir}js/qqbinding.js"></script>
