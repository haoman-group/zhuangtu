...
<body>
  ...
  <div class="productdetail_box">
    ...
    <div class="rightbox">
      ...
      <form id="form_add_worker"  method="post"  onsubmit="return checkForm()">
      ...
      <dl>
        ...
        <dd>
          <input type="text" name="crafttype" readonly value="{$categoryName}"></input>
        </dd>
        <dt> <i>*</i>
          从业年限：
        </dt>
...
<!--         <dt>好工人认证：</dt>
        <dd>
          <input type="hidden" name="goodworker" class="refreshclear" data-default-v="0" data-inpname="goodworker" value="<empty name='data.goodworker'>0<else/>{$data.goodworker}</empty>">
          <em data-v="1" data-radioname="goodworker_1" data-toinpname="goodworker" data-group="goodworker_group" class="<?php //echo ($data['goodworker'] ==1)?"bindinput radiogroup on":"bindinput radiogroup" ?>"></em>
          <label data-forradio="is_goodworker_1" class="forradio">是</label>
          <em data-v="0" data-radioname="goodworker_0" data-toinpname="goodworker" data-group="goodworker_group" class="<?php //echo ($data['goodworker'] ==0)?"bindinput radiogroup on":"bindinput radiogroup" ?>"></em>
          <label data-forradio="is_goodworker_0" class="forradio" >否</label>
        </dd> -->
<!--         <dt>自我评价：
        </dt>
        <dd>
          <textarea name="selfevalu" value="{$data.selfevalu}">{$data.selfevalu}</textarea>
        </dd> -->
        <dt> 
          施工经验：
        </dt>
...
<script type="text/javascript">
   ...
   //   var str = $('#' + returnid).html();
    ...
    function checkForm(){
      ...
      //   alert('请填写工人姓名!');
      //   $("input[name='workername'").focus().select();
      //   return false;
      // }
      // if($("input[name='workyears'").val()==""){
      //   alert('请填写从业年限!');
      //   $("input[name='workyears'").focus().select();
      //   return false;
      // }
      // if($("input[name='hometown'").val()==""){
      //   alert('请填写籍贯!');
      //   $("input[name='hometown'").focus().select();
      //   return false;
 ...
 function submitadd(){
  ...
  TableToJson();
    ...
    var url = "/Seller/Worker/" + action +"?id=" + id;
    $.ajax({
      url:url,
      type:"post",
      data:$("#form_add_worker").serialize(),
