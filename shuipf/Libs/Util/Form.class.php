...
class Form {
    ...
    public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $allowupload = 0, $allowbrowser = 1, $alowuploadexts = '', $allowuploadnum = '10', $height = 200, $disabled_page = 0) {
        ...
        elseif($toolbar == 'ztshopdecnor'){
            $toolbar = "['FullScreen', 'Source', '|','FontSize','Bold', 'Italic', 'forecolor', 'backcolor', 'Underline', 'strikethrough', 'fontborder', 'Link',  '|',  'InsertImage', 'pagebreak', 'removeformat', 'autotypeset', '|','insertorderedlist', 'insertunorderedlist', 'superscript', 'subscript', 'indent',
           'customstyle','justifyleft', 'justifycenter', 'justifyright', 'justifyjustify','inserttable','deletetable', 'preview']
                ";
        }
        elseif($toolbar == 'ztshopdecvip'){
            $toolbar = "['FullScreen', 'Source', '|','FontSize','Bold', 'Italic', 'forecolor', 'backcolor', 'Underline', 'strikethrough', 'fontborder', 'Link',  '|',  'InsertImage', 'pagebreak', 'removeformat', 'autotypeset', '|','insertorderedlist', 'insertunorderedlist', 'superscript', 'subscript', 'indent',
           'customstyle','justifyleft', 'justifycenter', 'justifyright', 'justifyjustify','inserttable','deletetable', 'emotion','insertvideo', 'preview']
                ";
        }
        $sess_id = time();
     ...
     */
    public static function date($name, $value = '', $isdatetime = 1, $loadjs = 0, $showweek = 'true', $timesystem = 1) {
        ...
        $str = '';
        $str .= '<input type="text" format="'.$format.'" time="'.$showsTime.'" name="' . $name . '" id="' . $id . '" value="' . $value . '" size="' . $size . '" class="input length_3 J_datetime ">';
        return $str;
    }
    ...
    public static function multi_images($name, $id = '', $value = '', $moudle = '', $catid = '', $size = 50, $class = 'input', $ext = '', $alowexts = '', $thumb_setting = array(), $watermark_setting = 0) {
        if (!$id)
            $id = $name;
        if (!$size)
            $size = 50;
        ...
        //1, 允许上传的文件类型, 是否允许从已上传中选择, 图片高度, 图片宽度,是否添加水印1是
        $authkey = upload_key("8,$alowexts,1,$thumb_ext,$watermark_setting");
        $str .= "<input type='button' onclick=\"flashupload('{$id}_images', '附件上传','{$id}',change_images,'8,{$alowexts},1,{$thumb_ext},{$watermark_setting}','{$moudle}','{$catid}','{$authkey}')\" value='添加图片' />";
        $str .= "<div id='{$id}'>";

        foreach ($value as $k => $v) {
            if(!empty($v)){
                 $str .= "<div id='{$id}_{$k}'> &nbsp<input type='text' name='{$name}_url[]' value='{$v}' style='width:310px;' ondblclick='image_priview(this.value);' class='input'> <input type='text' name='{$name}_alt[]' value='' style='width:160px;' class='input' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> <a href=\"javascript:remove_div('{$id}_{$k}')\">移除</a> </div>";
            }else{
            $str .= "<div id='null' style='display:none'> &nbsp<input type='text' name='null' value='{$v}' style='width:310px;' ondblclick='image_priview(this.value);' class='input'> <input type='text' name='{$name}_alt[]' value='' style='width:160px;' class='input' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> <a href=\"javascript:remove_div('{$id}_{$k}')\">移除</a> </div>";
        }
    }
        $str .= "</div>";
        return $str;
