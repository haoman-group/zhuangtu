...
class Local extends Passport {
    ...
    public function loginLocal($identifier, $password = null, $is_remember_me = 604800) {
        ...
        if ($this->registerLogin($userinfo, $is_remember_me)) {
            ...
            if(is_weixin() && $_SESSION['openid'] && empty($userinfo['weixin_unionid'])) {
                $data['weixin_unionid'] = $_SESSION['openid'];
            }

            D("Member/Member")->where($map)->save($data);
            //记录登陆日志
    ...
    public function getLocalUser($identifier, $password = null) {
        ...
        $user = $UserMode->where($map)->order('userid desc')->find();
        if (empty($user)) {
            $this->error = '该用户不存在！';
            return false;
        }
...
swfobject.addDomLoadEvent(function () {
     ...
     * @return boolean
     */
    public function userCheckMobile($mobile) {
    ...
    public function userRegister($username, $password, $mobile='') {
        ...
        //检查手机号码
        // $ckmobile = $this->userCheckMobile($mobile);
        // if ($ckmobile !== true) {
        //     return -2004;
        // }
    ...
    public function getUserAvatar($uid, $format = 90, $dbs = false) {
        ...
        if (!file_exists(SITE_PATH.$picurl)){
