...
class Local extends Attachment {
    ...
    public function uploadPic($Callback = false) {
        //是否生成缩略图
        if ($this->options['thumb']) {
            if ($this->options['thumbMaxWidth'] && $this->options['thumbMaxHeight']) {
                //开启生成缩略图
        ...
        if ($this->handler->upload($Callback)) {
            ...
            foreach ($info as $i => $value) {
                ...
                $aid = D('Picture/Picture')->fileInfoAdd($value, $this->options['module'], $this->options['catid'], $this->options['thumb'], $this->options['isadmin'], $this->options['userid'], $this->options['time']);
                if ($aid) {
                    $filePath = $value['savepath'] . $value['savename'];
                    $info[$i]['aid'] = $aid;
                    //附件完整访问地址
