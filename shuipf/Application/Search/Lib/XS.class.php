...
class XSIndex extends XSServer {
    ...
    public function addExdata($data, $check_file = true) {
        ...
        if ($first != CMD_IMPORT_HEADER && $first != CMD_INDEX_REQUEST && $first != CMD_INDEX_SYNONYMS && $first != CMD_INDEX_REMOVE && $first != CMD_INDEX_EXDATA) {
...
class XSSearch extends XSServer {
    ...
    private function logQuery($query = null) {
        ...
        if ($query !== '' && $query !== null) {
            ...
            $query = $this->_query;
            if (!$this->_lastCount || ($this->_defaultOp == CMD_QUERY_OP_OR && strpos($query, ' ')) || strpos($query, ' OR ') || strpos($query, ' NOT ') || strpos($query, ' XOR ')) {
                return;
            }
    ...
    private function preQueryString($query) {
        ...
        foreach ($parts as $part) {
            ...
            if (($pos = strpos($part, ':', 1)) !== false) {
                ...
                if (($field = $this->xs->getField($name, false)) !== false && $field->vno != XSFieldScheme::MIXED_VNO) {
            ...
            if (strlen($part) > 1 && ($part[0] == '+' || $part[0] == '-') && $part[1] != '(' && preg_match('/[\x81-\xfe]/', $part)) {
    ...
    private function regQueryPrefix($name) {
        if (!isset($this->_prefix[$name]) && ($field = $this->xs->getField($name, false)) && ($field->vno != XSFieldScheme::MIXED_VNO)) {
