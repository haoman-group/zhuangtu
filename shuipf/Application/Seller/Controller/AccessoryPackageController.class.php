...
class AccessoryPackageController extends SellerbaseController{
    ...
    public function add(){
      ...
      if(IS_POST){
        ...
        $this->display('AccessoryPackage/addaccessorypackage');
      }
    }
    ...
    private function _procData(){
       ...
       // $data['setup'] = serialize($data['setup']);
        ...
        $data['price_json']='{"0":{"计价方式":{"idx":"11","vid":"","txt":"根据实地测量","listno":0,"pictures":{}},"price":"1.00","price_act":"1.00","quantity":"21","tsc":"","barcode":"","hidden_value":"根据实地测量11"}}';
   ...
   public function accessorypackagelist(){
      $where['cid']=124886060;
      $where['uid']=$this->userid;
      $where['isdelete']=0;
      $where['status']=10;
      // $sellerbase= new SellerbaseController();
      $list=$this->_prepData($where);
      // $this->assign('list',$list);
   	$this->display();
   }

   public function accessorypackageshow(){
   	 $id=I('id');
   	 $data=$this->model->where(array('id'=>$id))->find();
