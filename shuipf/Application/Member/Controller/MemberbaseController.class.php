...
class MemberbaseController extends Base {
    ...
    final public function check_member() {
        ...
        //忘记密码时不检测登录
        if (ACTION_NAME == 'resetpwd' && CONTROLLER_NAME =="Account") {
            return true;
        }
        ...
        //特定方法无需登陆验证
        if (MODULE_NAME == 'Member' && CONTROLLER_NAME == 'Seller' && in_array(ACTION_NAME, array('login', 'register', 'logout', 'lostpassword', 'resetpassword', 'verifyemail','index','reg_sms','reg_mobile','doLogin'))) {
            //该类方法不需要验证是否登陆
            return true;
        }
         //特定方法无需登陆验证
        if (MODULE_NAME == 'Member' && CONTROLLER_NAME == 'Buyer' && in_array(ACTION_NAME, array('login', 'loginpop', 'register', 'logout', 'register1', 'register2', 'register3','register4','doLogin','findpsw','findpsw2','findpsw3','findpsw4'))) {
            //该类方法不需要验证是否登陆
            return true;
        }
        if(MODULE_NAME == "Member" && CONTROLLER_NAME == "Chat" && in_array(ACTION_NAME, array('index'))){
            return true;
        }
        ...
        if ($this->userid) {
            ...
            } else if ($this->userinfo['groupid'] == 7) {//邮箱认证
                //service("Passport")->logoutLocal();
            ...
            $this->redirect('Member/Seller/login');
        }
    }
