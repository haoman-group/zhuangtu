...
邮件服务器自动发送邮件请勿回信 {$date}';
    ...
    public function doVerifyEmail() {
        ...
        if (empty($userInfo)) {
            $this->message(array(
                'error' => 1100,
                'info' => '身份验证已经失效，请重新进行身份验证！',
            ));
        }
        $code = rand(100000,999999);//生成验证码
        $verifyword = 'Hi，{$username}:

你申请了修改绑定邮箱，新邮箱绑定所需验证码为：

'.$code.'

请在15分钟内完成验证码验证，负责验证码将会失效。

邮件服务器自动发送邮件请勿回信 {$date}';
        C("URL_MODEL", 0);
        $verifyword = str_replace(array(
            '{$username}',
            '{$date}',
        ), array(
            $userInfo['username'],
            date('Y-m-d H:i:s'),
        ), \Input::nl2Br($verifyword));

        if (SendMail($userInfo[2],  $userInfo[1] . '”在 ' . self::$Cache['Config']['sitename'] . ' 的验证码', $verifyword)) {
            $this->message(10000);
        } else {
            $this->error('邮件发送失败！');
        }
