...
class UserController extends MemberbaseController {
    ...
    public function change_mobile(){
        if(IS_POST){
            $mobile = I('mobile') or $this->error('参数不正确');
            $code = I('code') or $this->error('参数不正确');
            if($this->useinfo['mobile']==$mobile) $this->error('此手机号您已绑定');
            if(D('Member/Member')->where(array('mobile'=>$mobile))->find()) $this->error('此手机号已被绑定过了');
            $veriry = D('Member/Member')->reg_sms_verify($mobile,$code);
            if(!$veriry) $this->error('验证码不正确');
            D('Member/Member')->changeMobile($this->userid,$mobile);
            $this->success('修改成功');
        }else{
            $this->display();
        }
    ...
    public function sendsms(){
        $mobile = I('mobile') or $this->ajaxReturn(array('status'=>-1,'msg'=>'参数不正确'));
        if(!checkMobile($mobile)) $this->ajaxReturn(array('status'=>-2,'msg'=>'手机号码不正确'));
        $code = rand(100000,999999);
        $re = D('Member/Member')->reg_send_sms($code,$mobile);
        if(!$re) $this->ajaxReturn(array('status'=>-3,'msg'=>'发送验证码失败'));
