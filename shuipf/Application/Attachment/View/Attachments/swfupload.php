...
<style>
    ...
    .ulonlineimgs{ overflow: hidden;zoom:1; padding: 10px 0px;}
    .ulonlineimgs li{  float: left; width: 100px; height: 100px; margin-right: 10px; margin-bottom: 10px;border: 2px solid #1D76B7;}
    .ulonlineimgs img{ width: 100px; height: 100px; display: block;}
...
<script>
    ...
    function addurls(obj){
        ...
        var $inpurls= $(obj);
        var valurls= $inpurls.val().replace(/(^\s*)|(\s*$)/g, "");
        if(valurls.length === 0){
            alert("请输入网址");return false;
        }

        var arvalnum= valurls.split("\n");
        var arvlnumtrimed=[];
        var flag = true;
        ...
        $.each(arvalnum,function(i,v){
            if(flag){
                var nowurl= v.replace(/(^\s*)|(\s*$)/g, "");
                if(!regex.test(nowurl)){
                    alert("格式不符合");
                    return false;
                }
                arvlnumtrimed.push(nowurl);
            }

        })
        if(!flag){return false}
        if( (num + arvlnumtrimed.length -1) > file_upload_limit ){
            alert('不能选择超过'+file_upload_limit+'个附件'); return false;
        }
        if(num > file_upload_limit) {alert('不能选择超过'+file_upload_limit+'个附件'); return false;}
        $.each(arvlnumtrimed,function(i,v){
            $('#att-status').append('|'+v);
            $('#att-name').append('|'+v);
            $(".ulonlineimgs").append("<li><img src=\""+v+"\"></li>");
        });
...
<div class="wrap" style="padding:5px;">
    ...
    <div class="J_tabs_contents">
        ...
        <div class="pop_cont"> 请输入网络地址,多个网址请用换行隔开
            <div class="bk3"></div>
            <textarea style=" width: 610px;" onblur="addurls(this)"></textarea>
            <!--      <input type="text" name="info[filename]" class="input" value=""  style="width:600px;" placeholder="http://"  >-->
            <!--onblur="addonlinefile(this)"-->
            <!--      <input type="button" value="添 加" onclick="addurls(this)" style="display: inline-block; margin-top: 6px; padding: 5px;" >-->
            <ul class="ulonlineimgs"></ul>
        </div>
        <div class="pop_cont">
