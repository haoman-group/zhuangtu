...
class IndexController extends Base {
    ...
    public function index() {
        ...
        $posdata = D('Admin/PositionData')->getDataByPageID("7"); 
       
        foreach ($posdata['18'] as $key => $value) {
            $url[]=substr($value['url'],26);
        }
        foreach ($url as $k=> $v) {
            $uid[]=M('Shop')->where(array('domain'=>$v))->find();
           
        }
        foreach ($uid as $k1 => $v1) {    
            $posdata['18'][$k1]['leve']=M('Designer')->where(array('uid'=>$v1['uid']))->getField('qualification');
        }
        //获取设计师从业年限
        foreach ($posdata['18'] as $key => $value) {
            # code...
            $posdata['18'][$key]['workyears'] = M('Designer')->where(array('id'=>$value['data']['designer_name'],'status'=>'1'))->getField('work_time');
        }
        $lunbo1=$posdata['178'];
        $lunbo2=$posdata['204'];
