...
class SellerOrderController extends ApibaseController {
    ...
    protected function _initialize(){
        ...
        $this->shop_id =  M('Shop')->getFieldByUid($this->userid,'id');
    }
      //卖家订单列表
    public function lists(){
        //订单状态
        $order_state = I('order_state');
        if($order_state) $where['order_state'] = array('IN',$order_state);
        //卖家店铺ID
    ...
    private function _checkAuth($order_sn="",$rec_id=""){
        ...
        if(!empty($order_sn)){
            $order_shop_id = $this->orderModel->where(array('order_sn'=>$order_sn,'order_status'=>array(array('egt',10),array('elt',40)),'delete_state'=>0))->getField('shop_id');
            ...
            $order_shop_id = $this->orderGoodsmodel
                                               ...
                                               ->where(array('zt_order.order_state'=>array(array('egt',10),array('elt',40)),'zt_order.delete_state'=>0,'zt_order_goods.rec_id'=>$rec_id))
                                               ->getField('shop_id');
            if($order_shop_id != $this->shop_id){
    ...
    public function detail(){
        ...
        $order = $this->orderModel->getOrderInfo(array('order_sn'=>$order_sn,'shop_id'=>$this->shop_id),array('order_common','order_goods'));
        $shop_cat = D('Member/Shop')->getCatpidById($order['shop_id']);
        $order['order_state_txt'] = \Seller\Controller\Status::$OrderStatus[$shop_cat][$order['order_state']];
